...
\draw[rounded corners=5pt, postaction=decorate] (G) -| (I.north);
\draw[rounded corners=5pt, postaction=decorate] (H) -| (I.south);
\end{tikzpicture}
\caption{豪华名词}
\end{subfigure}
...
\end{tikzpicture}
\end{lrbox}
\sbox\TEMPBOX{\scalebox{.25}{\usebox\TEMPBOX}}
\edef\MYDEPTH{\the\dp\TEMPBOX}
\dp\TEMPBOX=0pt\relax
\begin{tikzpicture}[node distance=8mm and 7mm]
\node[circle, inner sep=1pt] (A) {开始};
\path[every node/.style={inner xsep=-3pt}]
  node (B) [above right= of A, yshift=-3mm] {\usebox\TEMPBOX};
...
\matrix (M)
  [row sep=5mm, inner sep=0pt, outer sep=0pt, matrix of nodes, nodes=fm]
  { 令$x=\fibo(n-1)$ \\ 令$y=\fibo(n-2)$ \\ };
\draw (M-1-1) -| ([xshift=-2mm]M.west) coordinate (L) |- (M-2-1)
      (M-1-1) -| ([xshift= 2mm]M.east) coordinate (R) |- (M-2-1);
\node[draw, circle, inner sep=1pt, left=12mm of M.south west]
  (A) {开始};
\node[left=2mm of A, inner sep=0pt] {$\fibo(n)$:};
\node[fm, right=10mm of R] (B) {$x+y$};
\node[draw, circle, inner sep=1pt, right=10mm of B.east|-A]
  (C) {结束};
\node[fm] (D) at ({$(L)!2!(L|-A)$} -| {$(A)!.5!(C)$}) {值是$1$};
\draw[postaction=decorate] (R) -- (B);
\tikzset{%
  every node/.style={font=\footnotesize, inner sep=0pt, outer sep=3pt},
  rounded corners=10pt}
\draw[postaction=decorate] (D) -| (C);
\draw[postaction=decorate] (B) -| (C);
\draw[postaction=decorate]
  (A) |- (D) node[near end, below] {若$n=1$或$2$};
\draw[postaction=decorate, decoration=reset marks, arrow decoration=0.65]
  (A) |- (L) node[near end, above] {若$n>2$};
\end{tikzpicture}
\caption[斐波那契数的RTN。]
  {一个斐波那契数的递归迁移网。}
\end{figure}
